...
import com.andfrankly.autotest.util.AssertMessages;
...
public class TestAndroidApp extends BaseTest {
	...
	public void login() {
		...
		Assert.assertEquals(AssertMessages.WELCOME_MESSAGE,
				welcomeLabel.getText());
	}

	@Test(dependsOnMethods = { "login" })
	public void verifyOnboarding() throws InterruptedException {
		PointOption swipeStart = PointOption.point(850, 789);
		PointOption swipeEnd = PointOption.point(150, 733);
		...
		Assert.assertEquals(AssertMessages.PAGE2_MESSAGE, pageLabel.getText());
		...
		logger.info("Verify page title: " + pageLabel.getText());
		Assert.assertEquals(AssertMessages.PAGE3_MESSAGE, pageLabel.getText());
		...
		logger.info("Verify page title: " + pageLabel.getText());
		Assert.assertEquals(AssertMessages.PAGE4_MESSAGE, pageLabel.getText());
	...
	public void verifySecondQuestionAndAddComment() throws InterruptedException {
		logger.info("Add comment to first question and skip (If you had the chance today, would you re-apply for your current job?)");
		...
		Assert.assertEquals(AssertMessages.THIRD_QUESTION_MESSAGE,
